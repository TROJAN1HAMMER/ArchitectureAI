...
+--------------------------------------------------------+
|                  Next.js 15 Frontend                   |
| (App Router, ThemeToggle, TreeView, GraphUI, SearchUI, AIChat, ArchAudit) |
+---------------------------+----------------------------+
                            |
                            | HTTP / REST (Axios)
                            v
...
| (Auth, Users, Health, Repos, Graph, Search, Ai, Arch)  |      | (Types, Schemas, Consts) |
+-----+---------------------+----------------------+-----+      +--------------------------+
      |                     |                      |
...
## Repository Intelligence, Knowledge Graph, Auditing & Grounded RAG Flow
...
RepositorySyncService
   ...
   ├── 9. Update RepositorySync record (status: SUCCESS)
   └── 10. Release Redis Lock safely (matching UUID)
   ↓
PostgreSQL (Repository, RepositoryFile, GraphNode, GraphEdge, Embedding, ArchitectureAnalysis, ArchitectureFinding, Conversation)
   ↓
Grounded RAG Pipeline (RagService)
   ...
   ├── 5. Retrieve Architecture Context (ArchitectureContextService)
   ...
   ↓
REST APIs (POST /repositories/:id/architecture/analyze, GET /findings, POST /ai/chat)
   ↓
Next.js Architecture Audit Tab (/repositories/[id])
```

## Folder Structure

- `frontend/`: React components, views, layout, client-side hooks, AI chat, and Architecture Audit UI.
- `backend/`: Core business logic services, controllers, AI/RAG services, architecture engine, middlewares, filters, and interceptors.
